...
10. **`PackageDataNormalizer`**: A new, critical component that validates package structures. It identifies reliable package hierarchies (from JARs or validated directory structures), labels them as `:ClassTree`, corrects `fqn` properties, and builds the clean `[:CONTAINS_CLASS]` hierarchy.
